...
export default function FilterBar() {
  ...
  return (
    <div className="flex items-center gap-3">
      <div
        className="scrollbar-none flex min-w-0 flex-1 items-center gap-2 overflow-x-auto py-1"
        style={{ maskImage: "linear-gradient(90deg, black 90%, transparent 100%)" }}
      >
        {PROPERTY_TYPES.map((type) => {
          const Icon = PROPERTY_TYPE_ICONS[type];
          return (
            <Pill key={type} active={selectedTypes.includes(type)} icon={<Icon size={15} />} onClick={() => toggleType(type)}>
      ...
      <select
        ...
        className="hidden shrink-0 cursor-pointer rounded-full border border-ink-200 bg-surface px-4 py-2 text-sm font-medium text-ink-700 sm:block"
      >
      ...
      <button
        ...
        className="flex shrink-0 items-center gap-2 rounded-full border border-ink-300 px-4 py-2 text-sm font-semibold text-ink-900 transition-shadow hover:shadow-md cursor-pointer"
      >
        <SlidersHorizontal size={15} />
        Filters
        {activeFilterCount > 0 && (
          <span className="brand-gradient flex h-5 w-5 items-center justify-center rounded-full text-xs text-white">
            {activeFilterCount}
          </span>
        )}
      </button>
